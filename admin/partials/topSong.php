...
function getArtist($artistId,$con){
    $artistQuery = mysqli_query($con,"SELECT name FROM artists WHERE id='$artistId' LIMIT 1");
    $artist =mysqli_fetch_array($artistQuery);
    return $artist['name'];
}
function getAlbum($albumId,$con){
    $albumQuery = mysqli_query($con,"SELECT title FROM albums WHERE id='$albumId' LIMIT 1 ");
    $album = mysqli_fetch_array($albumQuery);
    return $album['title'];

}
function getGenre($genreId,$con){
    $genreQuery = mysqli_query($con,"SELECT name FROM genres WHERE id='$genreId' LIMIT 1 ");
